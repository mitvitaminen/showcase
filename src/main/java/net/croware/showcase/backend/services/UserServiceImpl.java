...
public class UserServiceImpl implements IUserService {
    ...
    public Long countByUserFilter(Optional<UserFilterValue> userFilterValue) {
        ...
        if (ufv.getFilterString().isPresent() && ufv.getRoleFilter().isPresent()) {
        ...
        } else if (!ufv.getFilterString().isPresent() && ufv.getRoleFilter().isPresent()) {
            final Set<Role> roles = new HashSet<>();
            roles.add(ufv.getRoleFilter().get());
    ...
    public Stream<User> findByUserFilter(Optional<UserFilterValue> userFilterValue, Pageable pageable) {
        final UserFilterValue ufv = userFilterValue.orElseGet(() -> new UserFilterValue(null, null));
        ...
        if (ufv.getFilterString().isPresent() && ufv.getRoleFilter().isPresent()) {
            ...
            return Streams.concat(
                    this.userRepository.findByRolesInAndEmailContainingIgnoreCase(roles, filterString).stream(),
                    this.userRepository.findByRolesInAndFirstnameContainingIgnoreCase(roles, filterString).stream(),
                    this.userRepository.findByRolesInAndLastnameContainingIgnoreCase(roles, filterString).stream());
        } else if (!ufv.getFilterString().isPresent() && ufv.getRoleFilter().isPresent()) {
            final Set<Role> roles = new HashSet<>();
            roles.add(ufv.getRoleFilter().get());
